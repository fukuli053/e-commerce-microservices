                ...
                <Grid item style={{width: "100%", height: "fit-content"}}>
                    <Grid item xs={12}>
                        <form onSubmit={this.handleSubmit} className={classes.root}
                            ...
                            {renderFormTextField("Ad", "firstName")}
                            {renderFormTextField("Soyad", "lastName")}
                            {renderFormTextField("E-Posta", "email")}
                            {renderFormTextField("Açık adres", "addr essLine1")}

                            <Grid item container xs={11} sm={8}>
                                <Field
                                    name="addressLine2"
                                    component={renderReduxTextField}
                                    label="Adres Satırı 2 (isteğe bağlı)"
                            ...
                            {renderFormTextField("Şehir", "city")}
            ...
            <>
                ...
                {this.state.addressRemovalConfirmation ?
                    <ModalConfirmation closeModalHandler={this.closeModalHandler}
                                       ...
                                       title="Adresi Sil"
                                       question="Seçili adresi silmek istediğinize emin misiniz?"/> : null}
            </>
        )
    }
}
...
const validate = (formValues) => {
    ...
    requiredFields.forEach(field => {
        if (!formValues[field]) {
            errors[field] = 'Doldurulması gerekli alan.';
        }
    });
